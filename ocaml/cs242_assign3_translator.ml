...
let rec translate_term (t : Lang.Term.t) : IR.Term.t =
  ...
  | Lang.Term.Let (p, arg, body) ->
    ...
    | Lang.Pattern.Alias (ro, "x", tau)->
      let t1' = translate_term arg in
      let t' = translate_term(Lang.Term.Let(ro,arg,body)) in
      IR.Term.App(IR.Term.Lam("x",translate_type tau,t'),t1')
    | Lang.Pattern.Tuple (ro1,ro2) ->
      let t' = translate_term(
          Lang.Term.Let(ro1,Lang.Term.Project(arg,Ast.Left),Lang.Term.Let(ro2,Lang.Term.Project(arg,Ast.Right),body))) in
      t'
    | Lang.Pattern.TUnpack("X","x") ->
      let t1' = translate_term arg in
      let t2' = translate_term body in
      IR.Term.TUnpack("X","x",t1',t2')
    | _ -> IR.Term.Int 0
